...
public class DialogueResponseEvents : MonoBehaviour
{
    ...
    public void OnValidate()
    {
        ...
        if (events == null) // Auto generate prompts to put things in inspector
        {
            events = new ResponseEvent[dialogueObject.Responses.Length];
        }
        else // Risezes array so when you edit a dialogue object in the project tab it auto updates
        {
            Array.Resize(ref events, dialogueObject.Responses.Length);
        }

        for (int i = 0; i < dialogueObject.Responses.Length; i++) // Sets up the menu in the inspector
        {
