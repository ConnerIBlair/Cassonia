...
public class EnemyKnockback : MonoBehaviour
{
    ...
    public void KnockbackObject(Transform obj, Transform thisTrans, Vector3 optDir = new Vector3(), float optForce = 0)
    {
        if (optForce == 0) { optForce = force; }

        if (rbody != null) { return; } // makes sure not calling twice
        ...
        {
            optDir += thisTrans.position;
            getAngle(optDir, thisTrans);
        }
        else
        {
            getAngle(obj.position, thisTrans);
        }
        ...
        rbody.linearVelocity = -transform.right * optForce;
    ...
    private void getAngle(Vector3 obj2, Transform thisTrans)
    {
        differenceX = thisTrans.position.x - obj2.x;
        differenceY = thisTrans.position.y - obj2.y;
