...
{// Add slowing down as it goes further when thrown?
 // WORKING ON VELOCITY TO ADD SPEED TO PARTICLE EFFECTS
    ...
    {
        float x = target.x - transform.position.x; // x dist to travel
        float y = target.y - transform.position.y; // y dist to travel
        if (Mathf.Abs(x) > Mathf.Abs(y)) // horizontal or vertical
        {
            target = new Vector3(transform.position.x - x, target.y);

        } // will never be the same until it reaches the target because x and y never have the same amount to travel
        else
        {
            target = new Vector3(target.x, transform.position.y - y);
        }
    }
    ...
    {
        if (other.gameObject.CompareTag("Wall") || other.gameObject.CompareTag("Untagged"))
        {
            // If random idea for windseed is added, will need to check if collision is a jumpable or not
