...
public class PlayerItems : MonoBehaviour
{
    ...
    {
        if (player.currentState == PlayerState.attack || player.currentState == PlayerState.interact || player.Interactable != null || player.paused)
        {
            return;
        }
        if (Input.GetButtonDown("Item1") && use1) // Item1
        {
            ...
            Item1?.Use(player); //itemSlot1.myItem.myItem.Use(this.GetComponentInParent<PlayerMovement>());
        }
        if (Input.GetButtonDown("Item2") && use2) // Item2
        {
