...
public class PlayerSwordColliders : MonoBehaviour
{
    ...
    {
        if (other.CompareTag("Enemy") && other.TryGetComponent<EnemyHurtbox>(out EnemyHurtbox hurtbox))
        {
    ...
    private IEnumerator CHANGETHISCo(Collider2D other)
    {
        ...
        other.GetComponent<EnemyHurtbox>().Health(pAttack.damage);
        pAttack.StartCoroutine("AttackCo"); // Originally not here
        knockB.rbody = other.GetComponentInParent<Rigidbody2D>();
        knockB.KnockbackObject(other.transform.parent);
    }
