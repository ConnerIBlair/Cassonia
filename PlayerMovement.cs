...
public class PlayerMovement : ChangeSortingOrder
{
    ...
    private void Update()
    {
        ...
        if (Input.GetKeyUp(KeyCode.P))
        {
            ...
            // RaycastHit2D hit;
            //hit = Physics2D.Raycast(transform.position, lastDir, 3);
        ...
        if (Input.GetButtonUp("Interact")) //checked on up is important! No idea why...
        {
        ...
        if (currentState != PlayerState.interact && currentState != PlayerState.roll && currentState != PlayerState.jump)
        {
            change = Vector3.zero;
            ...
            if (Input.GetButtonDown("Attack"))
            {
                ...
                ChangeOrder(Sword.GetComponent<SpriteRenderer>());
